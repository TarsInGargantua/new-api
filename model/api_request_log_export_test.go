...
func TestAPIRequestLogExportBatchClaimsFullFilterWithoutDuplicates(t *testing.T) {
	...
	progress, err := UpdateAPIRequestLogExportBatchProgress(db, batch.Tag, "worker-a", 1)
	require.NoError(t, err)
	require.Equal(t, int64(1), progress.ProcessedRows)
	...
	require.Equal(t, batch.RowCount, completed.ProcessedRows)
