...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string, conversationId string) (logs []*Log, total int64, err error) {
	...
	tx = applyConversationIDFilter(tx, conversationId)
	if startTimestamp != 0 {
...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string, conversationId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	if logType == LogTypeUnknown {
...
func applyConversationIDFilter(tx *gorm.DB, conversationId string) *gorm.DB {
	...
	encoded, err := common.Marshal(conversationId)
	if err != nil {
		return tx
	}
